...
<?php
    ...
    {
        $id = $_GET['edit_id'];
        $stmt_edit = $DB_con->prepare('SELECT username,img,contact,email,password,type FROM adminlogin WHERE id =:id');
  ...
  $password = $_POST['password'];$type = $_POST['type'];
        ...
        if(!isset($errMSG))
        {
$stmt = $DB_con->prepare('UPDATE adminlogin SET username=:username,  img=:img,contact=:contact,  email=:email,password=:password, type= :type
    WHERE id=:id');
